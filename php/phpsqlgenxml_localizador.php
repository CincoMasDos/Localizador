...
if ($tipo=="parroquias") {
  ...
  // que no tiene arcip ni vicaría
        // También añadiéndole el campo status de la tabla node (que indica si un contenido está publicado o no)
  ...
  $query = "SELECT * 
         ...
         LEFT OUTER JOIN (
           SELECT nombre_comarca,id_comarca,tipo_poli
          FROM geo_comarcas
          HAVING (tipo_poli=0 OR tipo_poli=1)
         ) AS comarcas
         ON content_type_parroquias.field_prq_id_comarca_value = comarcas.id_comarca
         LEFT OUTER JOIN (
           SELECT nombre_arcip,id_arcip,tipo_poli
          FROM geo_arciprestazgos 
          HAVING (tipo_poli=0 OR tipo_poli=1)
         ) AS arciprestazgos
         ON content_type_parroquias.field_prq_id_arcip_value = arciprestazgos.id_arcip
         LEFT OUTER JOIN (
           SELECT nombre_vicaria,id_vicaria,tipo_poli
          FROM geo_vicarias HAVING (tipo_poli=0 OR tipo_poli=1)
         ) AS vicarias
         ON content_type_parroquias.field_prq_id_vicaria_value = vicarias.id_vicaria
         LEFT OUTER JOIN (
         ...
         ON content_type_parroquias.field_prq_id_value = geo_parroquias.field_prq_geo_id_value
                                 ...
                                 ON content_type_parroquias.nid = publicado.nid
         WHERE status = 1 AND field_prq_localizador_value = 1 AND content_type_parroquias.field_prq_iddiocesana_value=". $iddiocesana;
  //echo $query;
}
elseif ($tipo=="cmssv") {
        // En este caso, será content_type_cmss añadiéndole el campo status de la tabla node (que indica si un contenido está publicado o no)
        mysql_query("SET SQL_BIG_SELECTS=1"); // indicamos esto para ampliar la consulta y que permita
                                              // la relación de tablas de muchos registros
  $query = "SELECT * 
        FROM content_type_cmss 
        LEFT OUTER JOIN (
              SELECT nid,status
              FROM node
           ) AS publicado
        ON content_type_cmss.nid = publicado.nid
        WHERE status = 1 and content_type_cmss.field_cmss_iddiocesana_value=". $iddiocesana;
}
elseif ($tipo=="contenINTRA") {
        // En este caso, será content_type_contenedores añadiéndole el campo status de la tabla node (que indica si un contenido está publicado o no)
        mysql_query("SET SQL_BIG_SELECTS=1"); // indicamos esto para ampliar la consulta y que permita
                                              // la relación de tablas de muchos registros
  $query = "SELECT * 
        FROM content_type_contenedores 
        LEFT OUTER JOIN (
              SELECT nid,status
              FROM node
           ) AS publicado
        ON content_type_contenedores.nid = publicado.nid
        WHERE status = 1 and content_type_contenedores.field_conten_iddiocesana_value=". $iddiocesana;
}
elseif ($tipo=="arropa") {
        // En este caso, será content_type_tiendas_arropa añadiéndole el campo status de la tabla node (que indica si un contenido está publicado o no)
        mysql_query("SET SQL_BIG_SELECTS=1"); // indicamos esto para ampliar la consulta y que permita
                                              // la relación de tablas de muchos registros
  $query = "SELECT * 
           FROM content_type_tiendas_arropa
           LEFT OUTER JOIN (
              SELECT nid,status
              FROM node
           ) AS publicado
           ON content_type_tiendas_arropa.nid = publicado.nid
           WHERE status = 1 and content_type_tiendas_arropa.field_tiendas_iddiocesana_value=". $iddiocesana;
}
elseif ($tipo=="otrosRecursos") {
        // En este caso, será content_type_otros_recursos añadiéndole el campo status de la tabla node (que indica si un contenido está publicado o no)
        mysql_query("SET SQL_BIG_SELECTS=1"); // indicamos esto para ampliar la consulta y que permita
                                              // la relación de tablas de muchos registros
        $query = "SELECT *
           FROM content_type_otros_recursos
           LEFT OUTER JOIN (
              SELECT nid,status
              FROM node
           ) AS publicado
           ON content_type_otros_recursos.nid = publicado.nid
           WHERE status = 1 and content_type_otros_recursos.field_ot_iddiocesana_value=". $iddiocesana;
}
