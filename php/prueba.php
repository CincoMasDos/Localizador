...
elseif ($tipo=="cmssv") {
	$query = "SELECT * FROM content_type_cmss WHERE 1 and content_type_cmss.field_prq_iddiocesana_value=". $iddiocesana;
}
elseif ($tipo=="contenINTRA") {
...
elseif ($tipo=="arropa") {
	$query = "SELECT * FROM content_type_tiendas_arropa WHERE 1 and content_type_arropa.field_prq_iddiocesana_value=". $iddiocesana;
}
elseif ($tipo=="otrosRecursos") {
	$query = "SELECT * FROM content_type_otros_recursos WHERE 1 and content_type_otros_recursos.field_prq_iddiocesana_value=". $iddiocesana;
}
...
if ($tipo=="parroquias") {
	while ($row = @mysql_fetch_assoc($resultados)){  
	  ...
	  $nuevo_nodo = $nodo_padre->appendChild($nodoprq);  
		...
		$nuevo_nodo->setAttribute("horario1",utf8_encode($row['field_prq_horario1_value']));		
		$nuevo_nodo->setAttribute("atencion2",utf8_encode($row['field_prq_atencion2_value']));	
		$nuevo_nodo->setAttribute("horario2",utf8_encode($row['field_prq_horario2_value']));	
		$nuevo_nodo->setAttribute("atencion3",utf8_encode($row['field_prq_atencion3_value']));	
		$nuevo_nodo->setAttribute("horario3",utf8_encode($row['field_prq_horario3_value']));		
